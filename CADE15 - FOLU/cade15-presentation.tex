...
% \proofnode[above left=0.5cm and 0.05cm of n3] {n2} {$\eta_2$: $\e p(U,V)$};
 ...
 \proofnode[above left=0.25cm and -2cm of n3] {n1} {\alt<2-> {{\color{red} $\eta_1$: $r(Y),p(X,q(Y, b)),p(X,Y)\e $}}{$\eta_1$: $r(Y),p(X,q(Y, b)),p(X,Y)\e $}};
...
\begin{itemize}
	...
	\item solver asked to use only resolution and contraction rules
	\item 300s timeout
	\end{itemize}
...
\vspace{1cm}
Only 14/308 proofs were returned uncompressed
\end{frame}
